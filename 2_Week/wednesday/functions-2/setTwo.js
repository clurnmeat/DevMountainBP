...
// CODE HERE
const multiply = (num, num1, cb) => {
  let numbers = (num *= num1);
  return cb(numbers);
};
...
const uniqmodifier = (uniqArr) => {
  let uniqArrParam = uniqArr;
  console.log(
    `The new names array with all the duplicate items removed is ${uniqArrParam} `
  );
};
const newArr = uniq(names, uniqmodifier);
////////// PROBLEM 6 //////////
...
// CODE HERE
const nameAndIndex = (name, inde) => {
  console.log(`The item at index ${inde} is ${name}`);
};

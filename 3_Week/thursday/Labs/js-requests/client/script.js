...
const repeatMyParam = (event) => {
  ...
  axios
    .get(`${baseUrl}/repeat/{"string"}`)
    .then((response) => {
      console.log(response.data);
      repeatBtn.textContent = response.data;
...
const sendQuery = () => {
  let listOfQ = "banana";
  axios
    .get(`${baseUrl}/query-test?query=${listOfQ}`)
    .then((response) => {
      console.log(response.data);
    })
...
/*
    ...
    It console.logs("No query was entered")

    2: Send more than 1 query on the URL -- what happened?
    It counts and makes sure that it isn't over 1, if it is, console.logs("more than 1 query") 
*/
